...
then
echo "ENV JAVA_DOWNLOAD_URL https://download.java.net/java/GA/jdk14.0.1/664493ef4a6946b186ff29eb326336a2/7/GPL/openjdk-14.0.1_linux-x64_bin.tar.gz"
else
...
echo ""
#echo "RUN mkdir -p  \$JAVA_HOME;curl -jksSLH \"Cookie: oraclelicense=accept-securebackup-cookie\" \$JAVA_DOWNLOAD_URL --output \${JAVA_BASE_DIR}/jdk.tar.gz; ls -lrt \${JAVA_BASE_DIR}"
echo "RUN apt-get update -y; apt-get install -y curl"
echo "RUN mkdir -p  \$JAVA_HOME;curl \$JAVA_DOWNLOAD_URL --output \${JAVA_BASE_DIR}/jdk.tar.gz; ls -lrt \${JAVA_BASE_DIR}"
